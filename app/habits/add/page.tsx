...
export default function HabitConfigPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (payload.frequency === 'event') {
        if (payload.input_type === 'boolean') {
            if (!payload.condition_type.endsWith('_count')) payload.condition_type += '_count';
        } else if (eventScoringMethod === 'count') {
            if (!payload.condition_type.endsWith('_count')) payload.condition_type += '_count';
        }
      }
    ...
    >
        ...
        {currentStep === 0 && (
          <div className="space-y-4">
            ...
            {/* Frequency Card */}
            <Card className="rounded-2xl border border-border/40 shadow-sm bg-card transition-all">
            ...
            <Card className="rounded-2xl border border-border/40 shadow-sm bg-card overflow-hidden">
               <CardContent className="p-6 space-y-5">
                  <div className="flex items-center gap-2 text-primary">
                     <AlignLeft size={14} />
                     <span className="text-[10px] font-black uppercase tracking-[0.2em]">Input Type</span>
                  </div>
                  <div className="space-y-4">
             ...
             <Card className="rounded-2xl border border-border/40 shadow-sm bg-card overflow-hidden">
                <CardContent className="p-6 space-y-6">
                   <div className="flex items-center gap-2 text-primary">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em]">Success Condition</span>
                   </div>
                             ...
                             {formData.frequency === 'event' && (
                               <div className="mt-2 mb-2 bg-muted/40 border border-border/40 rounded-md p-2.5 flex items-start gap-2">
                                 ...
                                 <p className="text-[10px] text-muted-foreground/80 leading-relaxed font-medium">
                                   <strong className="text-foreground">Dual-Aggregation:</strong> Your target represents the {
                                     formData.input_type === 'boolean' 
                                       ? 'number of times you log YES' 
                                       : (eventScoringMethod === 'count' ? 'number of times this event is logged' : 'sum of all values logged')
                                   } today.
                                 </p>
                               </div>
                             )}
        ...
        <div className="flex gap-4 pt-4 pb-12">
          ...
          {currentStep === STEPS.length - 1 ? (
             <SaveButton isSaving={saving} label="Save Habit" className="flex-[2] h-11 bg-emerald-600 text-white rounded-md font-bold text-sm shadow flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:bg-muted" onClick={handleSave} />
          ) : (
            <Button 
