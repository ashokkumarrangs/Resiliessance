  ...
  const fetchData = async () => {
    ...
    try {
      ...
      fetchedConfigs
        ...
        .forEach((c) => {
          if (c.unlogged_is_success && !dataMap[c.habit_name]) {
  ...
  const handleSave = async () => {
    ...
    try {
      const dailyConfigs = configs.filter(
        (c) => c.frequency === "daily" || !c.frequency
      )
                      ...
                      {groupHabits.map((habit) => {
                        const status =
                          habit.frequency === "event"
                            ? eventAggregates[habit.habit_name]?.status ||
                              "Not Entered"
                        ...
                        return (
                          <div
                            key={habit.id}
                            className={`group/row flex items-center gap-2 rounded-xl p-2 transition-all ${styles.bg} ${styles.anim}`}
                          >
                                ...
                                <div className="flex w-20 shrink-0 justify-end">
                                  {habit.input_type === "boolean" ? (
                                    <Select
                                      ...
                                      className="h-8 w-full rounded-lg border-none bg-muted px-2 text-xs font-bold text-foreground shadow-inner transition-all focus:ring-2 focus:ring-primary/10"
                                    >
                                    ...
                                    <div className="flex w-full items-center justify-end gap-1">
                                      <input
                                        ...
                                        inputMode="numeric"
                                        className="h-8 w-7 rounded-lg border-none bg-muted p-0.5 text-center text-xs font-bold text-foreground focus:ring-2 focus:ring-primary/10"
                                        min="0"
                                      />
                                    ...
                                    <Input
                                      ...
                                      className="h-8 w-full rounded-lg border-none bg-muted text-center text-xs font-bold text-foreground focus:ring-2 focus:ring-primary/10"
                                      placeholder="--"
                                      inputMode={
                                        habit.input_type === "number"
                                          ? "decimal"
