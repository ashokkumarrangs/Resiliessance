...
export default function VehicleFuelServicePage() {
  ...
  const handleFuelSave = async () => {
    ...
    try {
      ...
      const [{ data: lastLog }, { data: lastFT }] = await Promise.all([
        supabase
      ...
      const ftOdo = lastFT && lastFT.length > 0 ? lastFT[0].odometer : 0;
    ...
    <div className="min-h-screen bg-background pb-32 font-dm-sans">
      <div className="max-w-lg mx-auto w-full p-4 md:p-6 pt-6 md:pt-6">
        ...
        <div className="-mt-2 mb-6">
          <SectionNav tabs={VEHICLE_TABS} activePath="/vehicles/fuel" />
        </div>
                 ...
                 <CardContent className="p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="space-y-1">
                          ...
                          <Input type="number" step="0.01" placeholder="Qty" value={fuelData.liters} onChange={e => setFuelData(p =>({...p, liters: e.target.value}))} className="h-12 rounded-md border border-border bg-muted/30 font-bold text-center" />
                       </div>
                       <div className="space-y-1">
