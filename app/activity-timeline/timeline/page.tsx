...
function ActivityTimelineContent() {
  ...
  const fetchTimelineData = async (dateStr: string) => {
    ...
    try {
      const summary = await activityService.getFullDaySummary(dateStr);
      const expenses = summary.expenses;
      const habits = summary.habits;
      const events = summary.events;
      const skillLogs = summary.skillLogs;
      const skillsConfig = summary.skillItems;
      const workouts = summary.workouts;
      const fuel = summary.fuelLogs;
      const service = summary.serviceLogs;
      const tasks = summary.tasks;
      const actionTasks = summary.actionTasks;
      const activityLogs = summary.activityLogs;
      const habitsConfig: any[] = [];
      const scheduledWorkouts: any[] = [];
      const petLogs: any[] = [];
      const petProfiles: any[] = [];
