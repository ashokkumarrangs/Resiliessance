...
export default function ReportsPage() {
  ...
  const [skillsDistTimeframe, setSkillsDistTimeframe] = useState<number>(180);
  const [skillsProgTimeframe, setSkillsProgTimeframe] = useState<number>(180);
  const [skillsTargetTimeframe, setSkillsTargetTimeframe] = useState<number>(180);
  const [skillsConsistencyTimeframe, setSkillsConsistencyTimeframe] = useState<number>(180);
  const [skillsDurationTimeframe, setSkillsDurationTimeframe] = useState<number>(180);
  ...
  const focusVsLegacyData = useMemo(() => {
    const today = new Date();
    const cutoffDate = subDays(today, skillsDistTimeframe);
    const cutoffStr = format(cutoffDate, "yyyy-MM-dd");
    ...
    return Object.entries(timeBySkill).map(([skillId, mins]) => {
  ...
  }, [skillLogs, skillItems, skillsDistTimeframe]);

  const consistencyHeatmap = useMemo(() => {
    const today = new Date();
    const cutoffDate = subDays(today, skillsConsistencyTimeframe);
    const cutoffStr = format(cutoffDate, "yyyy-MM-dd");
    const logsInTimeframe = skillLogs.filter(l => l.date >= cutoffStr);
  ...
  }, [skillLogs, skillsConsistencyTimeframe]);

  const progressionData = useMemo(() => {
    const today = new Date();
    const cutoffDate = subDays(today, skillsProgTimeframe);
    const cutoffStr = format(cutoffDate, "yyyy-MM-dd");
    ...
    return data;
  }, [skillLogs, skillsProgTimeframe, selectedProgressSkill]);

  const monthlyTargetData = useMemo(() => {
    const today = new Date();
    const cutoffDate = subDays(today, skillsTargetTimeframe);
    const cutoffStr = format(cutoffDate, "yyyy-MM-dd");
    ...
    return data;
  }, [skillItems, skillLogs, skillsTargetTimeframe]);

  const frequencyTrend = useMemo(() => {
    const today = new Date();
    const cutoffDate = subDays(today, skillsDurationTimeframe);
    const cutoffStr = format(cutoffDate, "yyyy-MM-dd");
    const logsInTimeframe = skillLogs.filter(l => l.date >= cutoffStr);
    ...
    return Object.entries(trendMap).map(([month, v]) => ({
      month,
      sessions: v.count,
  ...
  }, [skillLogs, skillsDurationTimeframe]);

  const skillsListTable = useMemo(() => {
    ...
    <div className="bg-background min-h-screen pb-20 p-4 md:p-6 font-dm-sans">
      <PageHeader title="Intelligence">
        ...
        <SectionNav tabs={[
          ...
          { title: "CORRELATIONS", icon: <LineChartIcon size={16} />, isActive: pathname.startsWith("/reports/correlations"), onClick: () => router.push("/reports/correlations") },
        ]} />
      </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <SectionCard 
                ...
                headerRight={
                  <Select 
                    value={skillsDistTimeframe} 
                    onChange={(e) => setSkillsDistTimeframe(parseInt(e.target.value))} 
                    className="bg-muted/30 border-none text-[8px] font-black uppercase text-primary rounded px-2 py-1"
                  >
                    <option value={30}>30 Days</option>
                    <option value={90}>90 Days</option>
                    ...
                    <ResponsiveContainer width="100%" height={160} className="max-w-[160px]">
                      <PieChart>
                        <Pie
                          ...
                          paddingAngle={3}
                        >
                          {focusVsLegacyData.map((entry, idx) => (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <StatPill 
                      ...
                      value={String(skillsListTable.reduce((s, item) => s + item.totalSessions, 0))}
                    />
                    <StatPill 
                      label="Total Practice" 
                      value={`${skillsListTable.reduce((s, item) => s + item.totalHours, 0)}h`}
                    />
                  </div>
                </div>
              </SectionCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <SectionCard 
                ...
                headerRight={
                  <div className="flex items-center gap-2">
                    <Select 
                      value={skillsProgTimeframe} 
                      onChange={(e) => setSkillsProgTimeframe(parseInt(e.target.value))} 
                      className="bg-muted/30 border-none text-[8px] font-black uppercase text-primary rounded px-2 py-1"
                    >
                      <option value={30}>30 Days</option>
                      <option value={90}>90 Days</option>
              ...
              <SectionCard 
                ...
                headerRight={
                  <Select 
                    value={skillsTargetTimeframe} 
                    onChange={(e) => setSkillsTargetTimeframe(parseInt(e.target.value))} 
                    className="bg-muted/30 border-none text-[8px] font-black uppercase text-primary rounded px-2 py-1"
                  >
                    <option value={30}>30 Days</option>
                    <option value={90}>90 Days</option>
            ...
            <SectionCard 
              title="Consistency Heat Grid" 
              icon={<Grid3X3 size={18} />}
              headerRight={
                <Select 
                  value={skillsConsistencyTimeframe} 
                  onChange={(e) => setSkillsConsistencyTimeframe(parseInt(e.target.value))} 
                  className="bg-muted/30 border-none text-[8px] font-black uppercase text-primary rounded px-2 py-1"
                >
                  <option value={30}>30 Days</option>
                  <option value={90}>90 Days</option>
            ...
            <SectionCard 
              title="Practice Frequency & Average Duration" 
              icon={<Clock size={18} />}
              headerRight={
                <Select 
                  value={skillsDurationTimeframe} 
                  onChange={(e) => setSkillsDurationTimeframe(parseInt(e.target.value))} 
                  className="bg-muted/30 border-none text-[8px] font-black uppercase text-primary rounded px-2 py-1"
                >
                  <option value={30}>30 Days</option>
                  <option value={90}>90 Days</option>
