  ...
  useEffect(() => {
    async function load() {
      ...
      const [
        ...
        supabase.from("habit_data").select("habit, value").gte("date", thisWeekStart).lte("date", thisWeekEnd),
        supabase.from("habit_data").select("habit, value").gte("date", lastWeekStart).lte("date", lastWeekEnd),
