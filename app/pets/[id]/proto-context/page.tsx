        ...
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-indigo-500/20 via-purple-500/10 to-transparent border border-indigo-500/20 p-8 flex flex-col items-center justify-center text-center shadow-2xl">
          ...
          <p className="text-sm font-semibold text-muted-foreground flex items-center gap-2">
             {calculateAge(petData.dob)} <span className="w-1 h-1 rounded-full bg-muted-foreground/50"></span> {petData.breed || "Pet"}
                ...
                {/* ACTION HUB - PROTOTYPE 2: CONTEXT */}
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-600 p-4 rounded-2xl mb-6 text-sm font-bold">
           Prototype 2: Contextual Menu. Click the main button to instant-log, or click the [ ... ] to see advanced options.
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-card border border-border/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4 overflow-visible">
            <h3 className="font-bold flex items-center gap-2"><Sparkles size={18} className="text-blue-500"/> Grooming</h3>
            <div className="flex flex-wrap gap-2">
               {['Bath', 'Nail Trim', 'Teeth'].map(x => (
                 <div key={x} className="relative flex items-center bg-blue-500/10 rounded-full group">
                   <button onClick={() => handleQuickLog(x)} className="px-4 py-2 text-blue-500 hover:bg-blue-500/10 active:scale-95 transition-all rounded-l-full text-xs font-black uppercase tracking-widest">{x}</button>
                   <button onClick={() => setCtxMenu(ctxMenu === x ? null : x)} className="pr-3 pl-2 py-2 text-blue-500 hover:bg-blue-500/10 rounded-r-full border-l border-blue-500/20">...</button>
                   {ctxMenu === x && (
                     <div className="absolute top-full left-0 mt-2 w-48 bg-card border shadow-xl rounded-xl z-50 py-2">
                        <button className="w-full text-left px-4 py-2 text-sm hover:bg-muted font-semibold">Log for Yesterday</button>
                        <button className="w-full text-left px-4 py-2 text-sm hover:bg-muted font-semibold">Add Notes / Photo</button>
                     </div>
                   )}
                 </div>
               ))}
