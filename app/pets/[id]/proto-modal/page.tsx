...
export default function PetDashboard({ params }: { params: Promise<{ id: string }> }) {
  ...
  const submitModalLog = async () => {
    if (!logModalTarget) return;
    try {
      const { error } = await supabase.from('pet_logs').insert({ pet_id: petId, log_type: logModalTarget, date: logDate, notes: logNotes });
      if (error) { toast.error("Error"); return; }
      toast.success(logModalTarget + " logged with details!");
      setLogModalTarget(null); setLogNotes(""); setLogDate(format(new Date(), 'yyyy-MM-dd'));
      fetchDashboardData();
    } catch(e) {}
  };
        ...
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-indigo-500/20 via-purple-500/10 to-transparent border border-indigo-500/20 p-8 flex flex-col items-center justify-center text-center shadow-2xl">
          ...
          <p className="text-sm font-semibold text-muted-foreground flex items-center gap-2">
             {calculateAge(petData.dob)} <span className="w-1 h-1 rounded-full bg-muted-foreground/50"></span> {petData.breed || "Pet"}
                ...
                {/* ACTION HUB - PROTOTYPE 1: MODAL */}
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-600 p-4 rounded-2xl mb-6 text-sm font-bold">
           Prototype 1: Draft & Confirm Bottom Sheet/Modal. Click any button to see the detailed entry form instead of an instant log.
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-card border border-border/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
            <h3 className="font-bold flex items-center gap-2"><Sparkles size={18} className="text-blue-500"/> Grooming</h3>
            <div className="flex flex-wrap gap-2">
               {['Bath', 'Nail Trim', 'Teeth'].map(x => <button key={x} onClick={() => setLogModalTarget(x)} className="px-4 py-2 bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 active:scale-95 transition-all rounded-full text-xs font-black uppercase tracking-widest">{x}</button>)}
            </div>
          </div>
          <div className="bg-card border border-border/40 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
            <h3 className="font-bold flex items-center gap-2"><Trees size={18} className="text-emerald-500"/> Activities</h3>
            <div className="flex flex-wrap gap-2">
               {['Beach Trip', 'Park Visit', 'Hike'].map(x => <button key={x} onClick={() => setLogModalTarget(x)} className="px-4 py-2 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 active:scale-95 transition-all rounded-full text-xs font-black uppercase tracking-widest">{x}</button>)}
            </div>
          </div>
        </div>

        {logModalTarget && (
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in slide-in-from-bottom-10 sm:fade-in-10">
            <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setLogModalTarget(null)}></div>
            <div className="relative bg-card w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 border shadow-2xl">
              <h3 className="font-bold mb-4 text-xl flex items-center gap-2"><Sparkles className="text-primary"/> Log {logModalTarget}</h3>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1 block">Date Performed</label>
                  <input type="date" value={logDate} onChange={e => setLogDate(e.target.value)} className="w-full bg-muted p-3 rounded-xl font-medium focus:outline-none" />
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1 block">Notes / Photo (Optional)</label>
                  <textarea value={logNotes} onChange={e => setLogNotes(e.target.value)} placeholder="E.g. Used the oatmeal shampoo" className="w-full bg-muted p-3 rounded-xl font-medium focus:outline-none min-h-[100px]"></textarea>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setLogModalTarget(null)} className="flex-1 bg-muted text-foreground p-4 rounded-xl font-bold">Cancel</button>
                <button onClick={submitModalLog} className="flex-1 bg-primary text-primary-foreground p-4 rounded-xl font-bold">Save Log</button>
              </div>
            </div>
          </div>
        )}
