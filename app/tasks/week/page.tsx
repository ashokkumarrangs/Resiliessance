...
export default function TaskManagerPage() {
            ...
            {showDone && completed.length > 0 && (
                <div className="space-y-2 pt-4 border-t border-border/20">
                    ...
                    {completed.map(task => {
                        ...
                        return (
                            <div key={task.id} className={`w-full relative ${isRelatedToActiveMenu(task.id) ? 'z-[60]' : 'z-10'}`}>
                                <div className={`grid gap-2 items-center bg-muted/10 border border-border/20 rounded-xl px-3 h-14 opacity-55 hover:opacity-90 transition-opacity group`} style={{ gridTemplateColumns: gridClass.includes("24px_40px") ? "22px 1fr 40px" : "22px 1fr 24px 40px" }}>
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <LoadingScreen message="Structuring weekly schedules..." />
      ) : (
