...
export default function TaskManagerPage() {
                    ...
                    return (
                        <div key={task.id} className={`w-full relative ${isRelatedToActiveMenu(task.id) ? 'z-[60]' : 'z-10'}`}>
                            <div 
                              ...
                              style={{ gridTemplateColumns: '24px 22px 1fr 24px 40px' }}
                            >
                                {/* Drag Handle */}
                ...
                <div className="space-y-2 pt-4 border-t border-border/20">
                    ...
                    {completed.map(task => {
                        ...
                        return (
                            <div key={task.id} className={`w-full relative ${isRelatedToActiveMenu(task.id) ? 'z-[60]' : 'z-10'}`}>
                                <div 
                                  className="grid gap-2 items-center bg-muted/10 border border-border/20 rounded-xl px-2 h-14 opacity-55 hover:opacity-90 transition-opacity group"
                                  style={{ gridTemplateColumns: '24px 22px 1fr 24px 40px' }}
                                >
                    ...
                    return (
                        <div key={task.id} className={`w-full relative ${isRelatedToActiveMenu(task.id) ? 'z-[60]' : 'z-10'}`}>
                            <div 
                              ...
                              className={`grid gap-2 items-center bg-card border border-border/40 border-l-4 rounded-xl px-2 h-14 shadow-sm transition-all group ${accentClass}`}
                              style={{ gridTemplateColumns: '24px 22px 1fr 40px' }}
                            >
                                {/* Drag Handle */}
                                <div className="p-1 text-muted-foreground/20 hover:text-primary rounded-md cursor-grab active:cursor-grabbing">
                                    <GripVertical size={16} />
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <LoadingScreen message="Loading inbox queue..." />
      ) : (
