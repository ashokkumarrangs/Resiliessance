...
function ReceivablesContent() {
  ...
  const [formData, setFormData] = useState({
    ...
    amount: "",
    type: "Pay", // Pay (lend) | Receive (collect)
  ...
  const fetchOptions = async () => {
    try {
      const [{ data: accs }, { data: recData }] = await Promise.all([
        supabase.from('liquidity').select('account_name'),
        supabase.from('receivables').select('party, party_type')
      ]);
      ...
      if (recData) {
        setOptions(prev => ({
          ...prev,
          parties: Array.from(new Set(recData.map(d => d.party).filter(Boolean))),
          partyTypes: Array.from(new Set(recData.map(d => d.party_type).filter(Boolean)))
        }));
      }
    } catch (error: any) {
      console.error("Error loading receivables options:", error);
  ...
  const handlePartySelect = async (party: string) => {
    ...
    const { data } = await supabase.from('receivables').select('*').eq('party', party).maybeSingle();
    if (data) {
      setPartyDetail(data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (partyDetail) {
        ...
        if (formData.type === 'Pay' && formData.category === 'Principal') {
          newTotal += amt;
          newRemaining += amt;
        } else if (formData.type === 'Receive' && formData.category === 'Principal') {
          newRemaining = Math.max(0, newRemaining - amt);
        } else if (formData.type === 'Receive' && formData.category === 'Interest') {
          newInterest += amt;
        }
      } else {
        newTotal = (formData.type === 'Pay' && formData.category === 'Principal') ? amt : 0;
        newRemaining = newTotal;
        newInterest = (formData.type === 'Receive' && formData.category === 'Interest') ? amt : 0;
      }

      // 1. Upsert receivable record
      if (partyDetail) {
      ...
      const { error: histErr } = await supabase.from('history_receivables').insert({
        date: formData.date,
        amount: amt,
        type: formData.type,
      ...
      // 3. Update liquidity (Receive = money in, Pay = money out)
      ...
      try {
        const actionLabel = formData.type === 'Pay' ? `Lent to ${formData.party}` : `Recovered from ${formData.party}`;
      ...
      toast.error(error.message || "Failed to save receivable");
    } finally {
  ...
  const handleCategoryChange = (val: string) => {
    if (val === 'Receivables') return;
    router.push(`/finance/capital/${val.toLowerCase()}`);
  };

  const directionBanner = formData.type === 'Pay'
    ...
    <PageWrapper title="Capital" reportHref="/reports/finance" sectionTabs={EXPENSE_TABS}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full mt-6">
        <div className="bg-card rounded-2xl p-5 shadow-sm border border-border/40 space-y-5">
          ...
          <PillToggle label="Type" options={['Pay', 'Receive']}
          ...
          {partyDetail && (
            <div className="bg-blue-50/60 rounded-xl p-4 space-y-2 border border-blue-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black uppercase text-muted-foreground tracking-wider">Outstanding Balance</p>
                  <p className="text-sm font-black text-foreground">₹{parseFloat(partyDetail.remaining || 0).toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase text-muted-foreground tracking-wider">Interest Received</p>
                  <p className="text-sm font-black text-amber-600">₹{parseFloat(partyDetail.interest_received || 0).toLocaleString()}</p>
                </div>
              </div>
            </div>
          )}
